...
describe Enumerable do
  ...
  describe '#my_each' do
    it 'returns each elements of the array if block is given' do
      expect(arr.my_each { |n| new_arr << n }).to eql(new_arr)
    end
  ...
  describe '#my_all?' do
    context 'Find all elements matching' do
      it 'when pattern provided if all elements are absolute equal to pattern returns true.' do
        expect(arr_str.my_all?(/a/)).to eql(true)
      end

      it 'when all items executed in block return true.' do
        expect(arr_str.my_all? { |n| n.size > 3 }).to eql(false)
      end

      it 'if any value is false when any pattern or block is not provided returns false .' do
  ...
  describe '#my_any?' do
    context 'Find any elements matching' do
      it 'when any items executed in block returns true.' do
        expect(arr_str.my_any? { |n| n.size > 3 }).to eql(true)
      end
  ...
  describe '#my_none?' do
    it 'when none of items executed in block returns true.' do
      expect(arr_str.my_none? { |n| n.size > 3 }).to eql(false)
    end
